...
app.post('/:id', (req,res) => {
    ...
    MongoClient.connect(url, {useNewUrlParser: true}, (err, db) => { 
        if(err) throw err;
        var dbo = db.db("MusicPlayer");
        dbo.collection("Musician").deleteOne({"_id":  objectId(id.id)}, (err, result) => {
            if(err) throw err;
            db.close();
            res.redirect('/')
        });
...
app.post('/:id/update',(req, res) => {
    ...
    MongoClient.connect(url, {useNewUrlParser: true}, (err, db) => { 
        if(err) throw err;
        var dbo = db.db("MusicPlayer");
        var newData = {$set : {name: req.body.newName}}
        var id = req.body
        dbo.collection("Musician").updateOne({"_id": objectId(id.delete)}, newData, (err, result) => {
            if(err) throw err;
            db.close();
            app.redirect('/')
        });
